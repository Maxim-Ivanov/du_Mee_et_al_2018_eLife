...
cuts <- read.table("Xu2009_CUTs_sacCer3.bed", sep="\t", \
header=F, stringsAsFactors=F)
cuts <- GRanges(seqnames=cuts$V1, ranges=IRanges(cuts$V2, \
end=cuts$V3, names=cuts$V4), strand=cuts$V6, \
seqinfo=seqinfo(Scerevisiae))

# Load all SUTs (n=847) from the same source:
suts <- read.table("Xu2009_SUTs_sacCer3.bed", sep="\t", \
header=F, stringsAsFactors=F)
suts <- GRanges(seqnames=suts$V1, ranges=IRanges(suts$V2, \
end=suts$V3, names=suts$V4), strand=suts$V6, \
seqinfo=seqinfo(Scerevisiae))

# Load all SGD genes (n=6692) from UCSC:
sgd <- read.table("sgdGene_UCSC.bed", sep="\t", header=F, \
comment.char="#", stringsAsFactors=F)
sgd <- GRanges(seqnames=sgd$V1, ranges=IRanges(sgd$V2, \
end=sgd$V3, names=sgd$V4), strand=sgd$V6, \
seqinfo=seqinfo(Scerevisiae))
...
valid <- width(gaps)<=1500 & width(gaps)>=100 & \
countOverlaps(gaps, all_genes, ignore.strand=T)==0
