...
bg_files <- batchReadTrackData(bg_filenames, seqinfo=seqinfo(Scerevisiae))
...
# division by zero:
dir_index <- rowSums(mcols(coding_cov)) / (rowSums(mcols(dnc_cov)) + 0.25)
...
write.table(output_df, "Directionality_of_CUT_downstream_genes.txt", sep="\t", quote=F)
