...
for file in *fastq.gz; do echo $file && trim_galore \
--length 15 --no_report_file $file; done

# Align trimmed reads to sacCer3 using STAR:
for file in *trimmed_fq.gz; do echo $file && STAR --genomeDir \
/index/saccer3/star --readFilesIn $file --runThreadN 4 \
--outFileNamePrefix ${file/fq.gz/} --outSAMmultNmax 1 \
--alignEndsType Local --readFilesCommand zcat; done

rm *out *tab; rmdir *STARtmp

# Convert SAM to sorted BAM:
for file in *sam; do echo $file && samtools view -hu $file | \
samtools sort - -o ${file/.sam/_sorted.bam}; done

# PAR-CLIP libraries are overloaded with rRNA reads. Filter out
# reads which were aligned to tRNA and rRNA genes (UCSC):
for file in *sorted.bam; do echo $file && bedtools intersect -v \
...
for file in *4sU*mapq10.bam; do
  ...
  reads=$(samtools flagstat $file | sed -n '1p' | \
awk '{print $1}')
  scaling=$( awk -v rds=$reads 'BEGIN{print 1000000 / rds}' )
  echo $sample $reads $scaling
  bedtools genomecov -ibam $file -bg -split -scale $scaling \
-strand "-" | awk 'BEGIN{OFS="\t"}{print $1, $2, $3, "-"$4}' \
> tempfile_rev && echo "  Reverse reads processed;"
  bedtools genomecov -ibam $file -bg -split -scale $scaling \
-strand "+" | cat - tempfile_rev | sort -k 1,1 -k 2,2n | \
sed "1i track type=bedGraph color=0,100,200 \
altColor=200,100,0" | gzip > $outfile
...
samtools merge $file 4sU_wt_rep1*mapq10.bam \
4sU_wt_rep2*mapq10.bam
bedtools genomecov -ibam $file -bg -split -strand "-" | \
awk 'BEGIN{OFS="\t"}{print $1, $2, $3, "-"$4}' > tempfile_rev
bedtools genomecov -ibam $file -bg -split -strand "+" | \
cat - tempfile_rev | sort -k 1,1 -k 2,2n | \
sed "1i track type=bedGraph color=0,100,200 \
altColor=200,100,0" | gzip > ${file/.bam/.bg.gz}
rm tempfile_rev
...
for file in *Nrd1_PAR-CLIP*mapq10.bam; do echo $file && \
samtools view -hb -F 16 $file > ${file/.bam/_fwReads.bam} && \
samtools view -hb -f 16 $file > ${file/.bam/_revReads.bam}; done

# Generate mpileup files:
samtools mpileup -C 50 -f /genomes/sacCer3.fa *fwReads.bam \
-o All_fwReads.mpileup
samtools mpileup -C 50 -f /genomes/sacCer3.fa *revReads.bam \
-o All_revReads.mpileup

# Call all SNPs from mpileup files:
java -Xmx8G -jar VarScan.v2.4.3.jar mpileup2snp \
All_fwReads.mpileup --min-coverage 6 --min-reads2 2 \
--min-var-freq 0.01 --min-avg-qual 5 --p-value 0.05 \
--strand-filter 0 --output-vcf 1 > All_fwReads.vcf
java -Xmx8G -jar VarScan.v2.4.3.jar mpileup2snp \
All_revReads.mpileup --min-coverage 6 --min-reads2 2 \
--min-var-freq 0.01 --min-avg-qual 5 --p-value 0.05 \
--strand-filter 0 --output-vcf 1 > All_revReads.vcf

# Extract the conversion events (T-to-C for the forward strand
# reads, A-to-G for the reverse strand reads):
sed '/^#/d' All_fwReads.vcf | awk '{if ($4=="T" && $5=="C") \
print}' > All_fwReads_TC_conv.vcf
sed '/^#/d' All_revReads.vcf | awk '{if ($4=="A" && $5=="G") \
print}' > All_revReads_AG_conv.vcf
...
for file in PAR-CLIP_signal*bg; do
  norm=$( sed '/^[#t]/d' $file | awk 'BEGIN{SUM=0}\
{SUM+=sqrt($4^2)*($3-$2)}END{print SUM / 1000000}' )
  echo $file $norm
  awk -v norm=$norm 'BEGIN{OFS="\t"}{if ($0~/^[#t]/) \
print $0; else print $1, $2, $3, $4 / norm}' $file | \
gzip > ${file/.bg/_norm1M.bg.gz}
done
