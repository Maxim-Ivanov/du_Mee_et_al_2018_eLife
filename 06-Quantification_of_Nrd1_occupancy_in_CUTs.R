...
source("normalizeBySmoothedSignal.R")
...
# pipeline):
thio_data <- import.bedGraph("4sU_wt_merged.bg.gz", \
...
parclip_filenames <- list.files(".", pattern = \
"^PAR-CLIP_signal.*norm1M.bg.gz")
parclip_data <- batchReadTrackData(parclip_filenames, \
seqinfo=seqinfo(Scerevisiae))
...
parclip_data_norm <- mendoapply(normalizeBySmoothedSignal, \
parclip_data, thio_data)

# Save the resultant GRangesList object as gzipped Bedgraph
# files:
writeBgWithCustomHeader <- function(gr, fname, header) {
  df <- subset(as.data.frame(gr), select=c("seqnames", \
"start", "end", "score"))
  extension <- paste(".", tools::file_ext(name))
  fname <- gsub(extension, "_norm4sU_1M.bg.gz", name)
  con <- file(fname, "w")
  writeLines(header, con)
  write.table(df, con, sep="\t", quote=F, row.names=F, \
col.names=F)
  close(con)
}
mapply(writeBgWithCustomHeader, parclip_data_norm, \
names(parclip_data_norm), parclip_headers)
...
write.table(as.data.frame(cuts_cov), \
file="Nrd1_PAR-CLIP_occupancy_of_68_CUTs.txt", sep="\t", quote=F)
